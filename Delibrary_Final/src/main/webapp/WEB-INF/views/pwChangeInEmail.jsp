...
<!-- 	<header id="page-header">
 ...
 <section id="pwChangePage">
        ...
        <div class="jumbotron text-center mt-5">
          ...
          <hr />
            ...
            <div class="form-group pw-changeForm m-auto">
              ...
              <input class="form-control mb-3" name="email" type="email" id="pwChange_email" placeholder="ex) hello@delibrary.com">
            </div>

            <div class="form-group pw-changeForm m-auto">
              <label for="password">새 비밀번호</label>
              <input class="form-control  mb-3" type="password" id="pwChange_pw" placeholder="비밀번호를 입력해주세요." name="pw">
            </div>
             ...
             <button class="btn btn-dark btn-lg mt-4" role="button" id="pwChangeExecute">비밀번호 변경하기</button>
          </div>
    </section>
  ...
  <script>
	...
	$('#pwChangeExecute').click(function() {
		console.log('pwChangeExecute OK');
		const email = $('#pwChange_email').val();
		const pw = $('#pwChange_pw').val();
		console.log(email);
		console.log(pw);
		
		$.ajax({
			url:'/pwChangeInEmail.do',
			type:'POST',
			data:{
				"email":email,
				"pw":pw
			...
			error:function(){
				 alert('비밀번호가 변경오류: 입력값을 다시 확인해 주세요.');
			}						
		});
	});	
    </script>
